...
static GraphicControllerFlags graphicControllerFlags;
...
GraphicControllerError graphicInit(void) 
{
    ...
    {
        printf("Error creating graphic controller thread\n");
...
void* graphicControllerTask(void* params)
{
    ...
    while (threadExit == 0)
    {
        if (graphicControllerFlags.draw == 1)
        {
            usleep(300000);

            graphicControllerFlags.draw = 0;

            /* check whether the screen should be black */
            if (graphicControllerFlags.drawBlack == 1)
            {
            ...
            fontDesc.height = 48;
            sprintf(channelString, "%d", graphicControllerFlags.channelNum);
            ...
            sprintf(audioPidString, "Audio PID: %d", graphicControllerFlags.audioPid);
            sprintf(videoPidString, "Video PID: %d", graphicControllerFlags.videoPid);
            fontDesc.height = 28;
            DFBCHECK(dfbInterface->CreateFont(dfbInterface, "/home/galois/fonts/DejaVuSans.ttf", &fontDesc, &fontInterface));
            DFBCHECK(primary->SetColor(primary, 0xff, 0x03, 0x03, 0xff));
        ...
        if (graphicControllerFlags.drawVolume == 1)
        {
            char volumePicture[50];

            graphicControllerFlags.drawVolume = 0;
            ...
            sprintf(volumePicture, "volume_%d.png", graphicControllerFlags.volume);
            printf("%s\n", volumePicture);
...
{
    return &graphicControllerFlags;
}

void clearScreen(void)
{
    /* clear the screen callback */
    if (graphicControllerFlags.drawBlack == 0)
    {
        primary->SetColor(primary, 0x00, 0x00, 0x00, 0x00);
        primary->FillRectangle(primary, 0, 0, screenWidth, screenHeight);
    }
